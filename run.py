...
state = translateState()#将数据类实例化

#切换翻译目标语言的函数
def lang_switch(state:translateState, args:str) -> None:
    if not args:
        print("没有输入要切换的语言，请输入“-lang 目标语言”，如“-lang 英文”") #防止没输参数
        return
    state.target_lang = args.strip() #让目标语言替换为命令输入的字符，#strip用于去除用户输入首尾的空格
    print(f"已经成功切换为{args}")
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            if not source_text:
                continue 
            #这个if用来防止用户什么也没输入，source_text是空字符串，那就直接跳过重来一轮输入

            if source_text.startswith('-'):#通过找-开头的字符串来识别命令
                cmd,_,args = source_text.partition(' ') 
                #这里必须用partition来切分为元组，不能用split，因为前者能拆分为固定的：命令cmd 分隔符_ 参数args
